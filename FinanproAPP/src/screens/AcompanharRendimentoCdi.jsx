...
const AcompanharRendimentoCdi = () => {
    ...
    const [rendimentoLiquidoCincoAnos, setRendimentoLiquidoCincoAnos] = useState(0);
    ...
    const calcularProjecoes = (amount, contribution = 0) => {
        ...
        const rendimentoAno = calcularRendimentoComAportes(amount, contribution, 252); // 252 dias úteis
        const rendimentoCincoAnos = calcularRendimentoComAportes(amount, contribution, 252 * 5); // 5 * 252 dias úteis
        const rendimentoDezAnos = calcularRendimentoComAportes(amount, contribution, 252 * 10); // 10 * 252 dias úteis
    ...
    const calcularRendimentoComAportes = (amount, contribution, days) => {
        ...
        for (let i = 0; i < days; i++) {
            ...
            if ((i + 1) % 21 === 0) { // Adiciona o aporte mensal a cada 21 dias úteis
                total += contribution;
            }
        }
        return total - amount;
    ...
    const renderSelectedOption = () => {
        switch (selectedOption) {
            case '1 Semana':
                return (
                    <>
                        <Text style={styles.result}>Projeção para 1 Semana: {formatCurrency(projecaoSemana)}</Text>
                        <Text style={styles.result}>Rendimento para 1 Semana: {formatCurrency(rendimentoSemana)}</Text>
                        <Text style={styles.result}>Rendimento Líquido para 1 Semana: {formatCurrency(rendimentoLiquidoSemana)} (descontando o imposto de renda sobre o lucro)</Text>
                    </>
                );
            case '1 Mês':
                return (
                    <>
                        <Text style={styles.result}>Projeção para 1 Mês: {formatCurrency(projecaoMes)}</Text>
                        <Text style={styles.result}>Rendimento para 1 Mês: {formatCurrency(rendimentoMes)}</Text>
                        <Text style={styles.result}>Rendimento Líquido para 1 Mês: {formatCurrency(rendimentoLiquidoMes)} (descontando o imposto de renda sobre o lucro)</Text>
                    </>
                );
            case '1 Ano':
                return (
                    <>
                        <Text style={styles.result}>Projeção para 1 Ano: {formatCurrency(projecaoAno)}</Text>
                        <Text style={styles.result}>Rendimento Líquido para 1 Ano: {formatCurrency(rendimentoLiquidoAno)} (descontando o imposto de renda sobre o lucro)</Text>
                    </>
                );
            case '5 Anos':
                return (
                    <>
                        <Text style={styles.result}>Projeção para 5 Anos: {formatCurrency(projecaoCincoAnos)}</Text>
                        <Text style={styles.result}>Rendimento Líquido para 5 Anos: {formatCurrency(rendimentoLiquidoCincoAnos)} (descontando o imposto de renda sobre o lucro)</Text>
                    </>
                );
            case '10 Anos':
                return (
                    <>
                        <Text style={styles.result}>Projeção para 10 Anos: {formatCurrency(projecaoDezAnos)}</Text>
                        <Text style={styles.result}>Rendimento Líquido para 10 Anos: {formatCurrency(rendimentoLiquidoDezAnos)} (descontando o imposto de renda sobre o lucro)</Text>
                    </>
                );
            default:
                return null;
    ...
    return (
        <View style={styles.container}>
            ...
            <Text style={styles.result}>Valor Atual: {formatCurrency(currentAmount)}</Text>
            <Picker
